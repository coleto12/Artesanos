...
export default function MyProducts() {
  ...
  return (
    <div style={{ ...s.page, padding: isMobile ? '24px 16px' : '56px', colorScheme: 'light' }}>
      ...
      {/* ENCABEZADO */}
      <div style={{ ...s.pageHeader, flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? '16px' : '0' }}>
        <div>
          <h1 style={s.pageTitle}>Mis productos</h1>
          <div style={s.titleBar} />
          <p style={s.pageSub}>Gestiona tu catálogo de artesanías</p>
        </div>
        <button style={{ ...s.btnAdd, width: isMobile ? '100%' : 'auto' }} onClick={() => {
      ...
      {/* FORMULARIO */}
      {showForm && (
        <div style={{ ...s.formCard, padding: isMobile ? '24px 16px' : '40px' }}>
        ...
        <div style={{
          ...
          {products.map(p => (
            <div key={p.id} style={s.card}>
              <div style={{ ...s.imgBox, height: isMobile ? '140px' : '200px' }}>
                {p.image
                  ? <img src={getMediaUrl(p.image)} alt={p.name} style={s.img} />
                  : <div style={s.noImg}>🏺</div>}
              </div>
              <div style={{ ...s.cardBody, padding: isMobile ? '12px' : '18px 20px' }}>
                <p style={s.cardCategory}>{p.category_name || 'Sin categoría'}</p>
                <h3 style={{ ...s.cardName, fontSize: isMobile ? '0.9rem' : '1.05rem' }}>{p.name}</h3>
                <p style={s.cardPrice}>${Number(p.price).toLocaleString('es-CO')}</p>
                <p style={s.cardStock}>Stock: {p.stock}</p>
...
const s = {
  page: { fontFamily: "'Segoe UI', sans-serif", background: '#fff', minHeight: '100vh', boxSizing: 'border-box', color: '#222' },
  pageHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '40px' },
  pageTitle: { fontFamily: "'Playfair Display', serif", fontSize: '2.2rem', fontWeight: 700, margin: '0 0 10px', color: '#1a1a1a' },
  titleBar: { width: '48px', height: '3px', background: GOLD, marginBottom: '12px' },
  pageSub: { color: '#888', fontSize: '0.95rem', margin: 0 },
  btnAdd: { background: BEIGE, color: '#fff', border: 'none', padding: '12px 28px', borderRadius: '4px', fontSize: '0.95rem', cursor: 'pointer', fontWeight: 600 },
  formCard: { background: '#F6F1E7', borderRadius: '12px', marginBottom: '48px', border: '1px solid #e8e0d0' },
  formTitle: { fontFamily: "'Playfair Display', serif", fontSize: '1.4rem', fontWeight: 700, margin: '0 0 24px', color: '#1a1a1a' },
  ...
  grid: { display: 'grid' },
  card: { border: '1px solid #eee', borderRadius: '10px', overflow: 'hidden', background: '#fff' },
  ...
  cardBody: { },
  cardCategory: { fontSize: '0.72rem', color: '#aaa', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 6px' },
  cardName: { fontFamily: "'Playfair Display', serif", fontWeight: 700, margin: '0 0 8px', color: '#1a1a1a' },
  cardPrice: { fontSize: '1.05rem', fontWeight: 700, color: '#222', margin: '0 0 4px' },
  cardStock: { fontSize: '0.82rem', color: '#aaa', margin: '0 0 12px' },
  cardBtns: { display: 'flex', gap: '8px' },
  btnEdit: { flex: 1, padding: '8px', background: '#F6F1E7', color: '#555', border: '1px solid #ddd', borderRadius: '4px', cursor: 'pointer', fontSize: '0.82rem', fontWeight: 600 },
  btnDelete: { flex: 1, padding: '8px', background: '#fff', color: '#c0392b', border: '1px solid #f5c6c6', borderRadius: '4px', cursor: 'pointer', fontSize: '0.82rem', fontWeight: 600 },
  ...
  emptyText: { fontFamily: "'Playfair Display', serif", fontSize: '1.3rem', color: '#1a1a1a', margin: '0 0 8px' },
