...
export default function Stories() {
  ...
  return (
    <div style={{ ...s.page, colorScheme: 'light' }}>
      ...
      {/* HERO */}
      <div style={{ ...s.hero, padding: isMobile ? '32px 20px 24px' : '56px 56px 40px' }}>
        <p style={s.heroLabel}>Patrimonio cultural</p>
        <h1 style={{ ...s.heroTitle, fontSize: isMobile ? '1.8rem' : '2.2rem' }}>
          Historias que tejen Colombia
        </h1>
        <p style={s.heroSub}>
      ...
      {!filter && (
        <div style={{
          ...
          {!isMobile && <div style={s.featNum}>01</div>}
        </div>
      )}

      {/* SECCION */}
      <div style={{ ...s.section, padding: isMobile ? '0 16px 48px' : '0 56px 64px' }}>
        ...
        <div style={{
          ...
          {filtered.map((story) => (
            <div key={story.id} style={s.card} onClick={() => setSelected(story)}>
              <p style={s.cardNum}>{pad(story.id)}</p>
              <h3 style={{ ...s.cardTitle, fontSize: isMobile ? '0.88rem' : '0.95rem' }}>
                {story.title}
              </h3>
              <p style={s.cardRegion}> {story.region}</p>
              <p style={s.cardCraft}>{story.craft}</p>
              {!isMobile && <p style={s.cardPreview}>{story.content.slice(0, 90)}…</p>}
      ...
      {selected && (
        <div style={s.overlay} onClick={() => setSelected(null)}>
          <div style={{
            ...
            <h2 style={{ ...s.modalTitle, fontSize: isMobile ? '1.2rem' : '1.4rem' }}>
              {selected.title}
            </h2>
...
const s = {
  ...
  hero: { borderBottom: '1px solid #eee', marginBottom: '32px' },
  heroLabel: { fontSize: '11px', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#aaa', marginBottom: '12px' },
  heroTitle: { fontFamily: "'Playfair Display', serif", fontWeight: 600, color: '#1a1a1a', marginBottom: '12px' },
  heroSub: { fontSize: '0.95rem', color: '#666', lineHeight: 1.7, maxWidth: '520px', marginBottom: '24px' },
  searchWrap: { position: 'relative', display: 'flex', alignItems: 'center', width: '100%', maxWidth: '360px' },
  searchIcon: { position: 'absolute', left: '12px', fontSize: '14px', pointerEvents: 'none' },
  searchInput: { width: '100%', padding: '9px 14px 9px 36px', fontSize: '0.9rem', border: '1px solid #ddd', borderRadius: '8px', outline: 'none', fontFamily: 'inherit', color: '#222', background: '#fff', boxSizing: 'border-box' },
  featured: { display: 'grid', gap: '2rem', alignItems: 'start', background: '#f9f7f4', borderRadius: '12px', border: '1px solid #eee' },
  featContent: { flex: 1 },
  featLabel: { fontSize: '11px', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#aaa', marginBottom: '8px' },
  featRegion: { fontSize: '12px', color: '#999', marginBottom: '8px' },
  featTitle: { fontFamily: "'Playfair Display', serif", fontWeight: 600, color: '#1a1a1a', marginBottom: '10px', lineHeight: 1.4 },
  featCraft: { display: 'inline-block', fontSize: '11px', letterSpacing: '0.08em', textTransform: 'uppercase', padding: '3px 10px', borderRadius: '6px', background: '#fff', border: '1px solid #ddd', color: '#666', marginBottom: '14px' },
  featText: { fontSize: '0.9rem', color: '#555', lineHeight: 1.75, marginBottom: '20px' },
  featBtn: { fontSize: '13px', color: '#1a1a1a', border: '1px solid #ccc', background: 'none', padding: '8px 18px', borderRadius: '8px', cursor: 'pointer', fontFamily: 'inherit' },
  featNum: { fontFamily: "'Playfair Display', serif", fontSize: '56px', fontWeight: 600, color: '#e0dcd6', lineHeight: 1, paddingTop: '8px', userSelect: 'none' },
  section: { },
  sectionHeader: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' },
  sectionTitle: { fontFamily: "'Playfair Display', serif", fontSize: '1.2rem', fontWeight: 600, color: '#1a1a1a' },
  count: { fontSize: '13px', color: '#aaa' },
  grid: { display: 'grid', border: '1px solid #eee', borderRadius: '12px', overflow: 'hidden' },
  card: { padding: '16px', background: '#fff', cursor: 'pointer', borderRight: '1px solid #eee', borderBottom: '1px solid #eee', transition: 'background 0.15s' },
  cardNum: { fontSize: '11px', color: '#bbb', marginBottom: '6px' },
  cardTitle: { fontFamily: "'Playfair Display', serif", fontWeight: 600, color: '#1a1a1a', lineHeight: 1.4, marginBottom: '6px' },
  cardRegion: { fontSize: '12px', color: '#999', marginBottom: '4px' },
  cardCraft: { fontSize: '11px', letterSpacing: '0.07em', textTransform: 'uppercase', color: '#888', marginBottom: '10px' },
  cardPreview: { fontSize: '13px', color: '#666', lineHeight: 1.6, marginBottom: '12px' },
  cardLink: { fontSize: '12px', color: '#1a1a1a' },
  overlay: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' },
  modal: { background: '#fff', borderRadius: '14px', border: '1px solid #eee', maxWidth: '500px', width: '100%', position: 'relative', maxHeight: '85vh', overflowY: 'auto' },
  modalClose: { position: 'absolute', top: '16px', right: '16px', background: 'none', border: '1px solid #ddd', borderRadius: '8px', padding: '4px 12px', cursor: 'pointer', fontSize: '13px', color: '#888', fontFamily: 'inherit' },
  modalNum: { fontSize: '11px', color: '#aaa', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '8px' },
  modalTitle: { fontFamily: "'Playfair Display', serif", fontWeight: 600, color: '#1a1a1a', marginBottom: '16px', lineHeight: 1.4 },
  modalDivider: { height: '1px', background: '#eee', marginBottom: '16px' },
  modalTags: { display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '20px' },
  tag: { fontSize: '12px', padding: '4px 12px', borderRadius: '6px', border: '1px solid #eee', color: '#666', background: '#f9f7f4' },
  modalText: { fontSize: '0.95rem', color: '#555', lineHeight: 1.8, marginBottom: '24px' },
  modalBtn: { fontSize: '13px', border: '1px solid #ccc', background: 'none', padding: '8px 18px', borderRadius: '8px', cursor: 'pointer', color: '#1a1a1a', fontFamily: 'inherit' },
}